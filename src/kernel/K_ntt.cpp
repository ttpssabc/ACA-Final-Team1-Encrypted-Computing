...
#pragma HLS PIPELINE
 		...
 		for (ap_uint<LOG_RNS+1> r = 0; r < (RNS_NUM-1); r++) {
 			for (ap_uint<LOG_N+1> i = 0; i < N/2; i++) {
#pragma HLS PIPELINE
 				out[i+r*N]     = modular_multiplication(poly_coeff[r][i/(N/UNROLL_NUM_NTT/2)][i%(N/UNROLL_NUM_NTT/2)](PRIME_BIT-1, 0),
 						                                N_inv[r], MOD[r], r);
 				out[i+N/2+r*N] = modular_multiplication(poly_coeff[r][i/(N/UNROLL_NUM_NTT/2)][i%(N/UNROLL_NUM_NTT/2)](2*PRIME_BIT-1, PRIME_BIT),
 					                                	N_inv[r], MOD[r], r);
 			}
 		}
 		for (ap_uint<LOG_RNS+LOG_N+1> i = (RNS_NUM-1)*N; i < (RNS_NUM)*N; i++) {
...
#pragma HLS PIPELINE
 		...
 		for (ap_uint<LOG_RNS+1> r = 0; r < RNS_NUM; r++) {
 			for (ap_uint<LOG_N+1> i = 0; i < N/2; i++) {
#pragma HLS PIPELINE
 				ap_int<LOG_RNS+1> idx = r + RNS_NUM;
 				out[i+r*N]     = modular_multiplication(poly_coeff[r][i/(N/UNROLL_NUM_NTT/2)][i%(N/UNROLL_NUM_NTT/2)](PRIME_BIT-1, 0),
 						                                N_inv[idx], MOD[idx], idx);
 				out[i+N/2+r*N] = modular_multiplication(poly_coeff[r][i/(N/UNROLL_NUM_NTT/2)][i%(N/UNROLL_NUM_NTT/2)](2*PRIME_BIT-1, PRIME_BIT),
 					                                	N_inv[idx], MOD[idx], idx);
 			}
 		}
